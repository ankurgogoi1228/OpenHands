...
DESIGN
    ...
    candidates              m   = 10
    committee sizes         k   = 2, 3, 4, 5, 6, 7, 8, 9   (k = 1 is a single winner;
                                k = m takes everything)
...
SCORE DISCIPLINE (STRICT)
    ...
    - Rank -> score map: (5, 4, 3, 2, 0) for m = 5 (the paper's map), (5, 4, 3, 2, 1, 0, 0)
      for m = 7 and (5, 5, 4, 4, 3, 3, 2, 2, 1, 0) for m = 10. Six grades cannot rank more
      than six candidates without ties, so the larger maps repeat grades; every map stays
      non-increasing, keeps the top grade first, the bottom grade last, and uses all six
      grades. See RANK_MAPS in the configuration block.
...
CURVE SMOOTHING (DISPLAY ONLY)
    A panel can carry up to 3 rules x 8 committee sizes, so the plotted curves are drawn
    with a centred, edge-corrected moving average of SMOOTH_WINDOW points (default 5) that
    takes the Monte Carlo jitter out of the picture. Only the drawing is smoothed: the CSV,
    the tables and every audit number keep the raw values. Set SMOOTH_WINDOW = 1 to plot the
    raw points, and raise --trials for curves that are genuinely less noisy.
...
Command line
------------
    python sim1_intensity_sweep_m10.py                    # full run, 1000 trials per cell
    python sim1_intensity_sweep_m10.py --trials 100       # quick test run
    python sim1_intensity_sweep_m10.py --skip-figures     # data and tables only
    python sim1_intensity_sweep_m10.py --skip-simulation  # re-plot an existing CSV
========================================================================================
"""
import argparse
import itertools
...
# The Shapley start-up check compares the vectorised rule with the reference loop for the
# first, middle and last committee size; probing a few k values keeps start-up quick when
# there are many of them.
SHAPLEY_PROBE_KS = (COMMITTEE_SIZES if len(COMMITTEE_SIZES) <= 5 else
                    [COMMITTEE_SIZES[0], COMMITTEE_SIZES[len(COMMITTEE_SIZES) // 2],
                     COMMITTEE_SIZES[-1]])
...
# Rank -> score maps, strict integers in {0,...,5}. m = 5 uses the paper's map verbatim;
# the larger maps repeat grades, because six grades cannot rank more than six candidates
# without at least one tie. Every map is non-increasing, gives the top grade to the best
# rank and the bottom grade to the worst rank, and uses all six grades.
RANK_MAPS = {5: (5, 4, 3, 2, 0), 7: (5, 4, 3, 2, 1, 0, 0),
             10: (5, 5, 4, 4, 3, 3, 2, 2, 1, 0)}
RANK_TO_SCORE = np.array(RANK_MAPS[M], dtype=int)
assert len(RANK_TO_SCORE) == M, "rank -> score map must have one entry per candidate"
assert np.all(np.diff(RANK_TO_SCORE) <= 0), "rank -> score map must be non-increasing"
assert RANK_TO_SCORE[0] == SCALE_MAX and RANK_TO_SCORE[-1] == 0, \
    "rank -> score map must run from the top grade to the bottom grade"
assert len(set(RANK_TO_SCORE.tolist())) >= 5, "rank -> score map is too coarse"
INTEGER_MEDIAN = True
...
def smart_median(arr, axis=0):
    ...
    if n % 2 == 1:
        return np.median(arr, axis=axis)
    kth = n // 2 - 1
    # np.partition puts the wanted order statistic in its sorted position and is cheaper
    # than sorting the whole (voters x subsets) matrix that the Shapley rule builds
    return np.take(np.partition(arr, kth, axis=axis), kth, axis=axis).astype(float)
...
def run_culture(args):
    ...
    for _ in range(trials):
        ...
        for ix, n_manip in enumerate(MANIP_COUNTS):
            ...
            # A rule only orders the candidates -- k slices that order and never changes it,
            # and the bottom target is the same candidate for every k. So each rule is
            # evaluated once per intensity: the honest order, the bottom-attack order, and
            # the cutoff-attack order of each k (the only k-dependent evaluation, because
            # the cutoff target r_h[k] moves with k). Every recorded cell is then read off
            # these orders, which shares one set of expensive Shapley values across all k.
            rank_h = [fn(honest, 1)[0] for fn in RULE_FUNCTIONS]
            rank_bottom = []
            for ir, rule_fn in enumerate(RULE_FUNCTIONS):
                target = rank_h[ir][M - 1]
                attacked[n_honest:, target] = SCALE_MAX
                rank_bottom.append(rule_fn(attacked, 1)[0])
                attacked[n_honest:, target] = 0
            for ik, k in enumerate(COMMITTEE_SIZES):
                honest_sets = [set(r[:k]) for r in rank_h]
                for it, t_type in enumerate(TARGETS):
                    for ir, rule_fn in enumerate(RULE_FUNCTIONS):
                        c_h = honest_sets[ir]
                        target = rank_h[ir][k] if t_type == 'Cutoff' else rank_h[ir][M - 1]
                        if t_type == 'Cutoff':
                            attacked[n_honest:, target] = SCALE_MAX
                            r_post = rule_fn(attacked, 1)[0]
                            attacked[n_honest:, target] = 0
                        else:
                            r_post = rank_bottom[ir]
                        c_post = set(r_post[:k])
                        if c_h != c_post:
...
def audit_results(df, base_dir):
    ...
    check(f"all committee sizes k = {_k_range()}", sorted(df.k.unique()) == COMMITTEE_SIZES,
    ...
    cut_exact = float((cut.abs() < 1e-9).mean()) * 100
    check("cutoff attack: k-Median change rate == success rate (>= 99% of cells)",
          cut_exact >= 99.0,
          f"{cut_exact:.1f}% of the {len(cut)} cells match exactly; largest excess "
          f"{float(cut.max()):.1f} pp")
...
plt.rcParams.update({
    ...
    # fine-grained drawing: no path simplification and no pixel snapping of flat segments,
    # so dense curves keep every plotted point instead of looking facetted
    "path.simplify": False, "path.simplify_threshold": 0.0, "path.snap": False,
...
K_LINESTYLES = {2: (0, (1, 1.6)), 3: (0, (5, 2)), 4: "-", 5: (0, (4, 1.5, 1, 1.5)),
                6: (0, (8, 2)), 7: (0, (2, 1.2)), 8: (0, (1, 1, 3, 1)),
                9: (0, (6, 1.5, 1, 1.5))}
K_MARKERS = {2: "o", 3: "s", 4: "^", 5: "D", 6: "v", 7: "<", 8: ">", 9: "P"}
K_LABELS = {2: r"$k=2$", 3: r"$k=3$", 4: r"$k=4$", 5: r"$k=5$", 6: r"$k=6$",
            7: r"$k=7$", 8: r"$k=8$", 9: r"$k=9$"}
K_COLORS = {2: "#4C72B0", 3: "#DD8452", 4: "#55A868", 5: "#8172B3", 6: "#937860",
            7: "#DA8BC3", 8: "#8C8C8C", 9: "#CCB974"}
MARK_AT = [1, 25, 49]      # marker anchors: any more would clutter a panel with 8 k values
...
def _k_range():
    """The committee sizes actually simulated, e.g. '2,3,4,5,6'."""
    return ",".join(str(k) for k in COMMITTEE_SIZES)


def _design_note(df):
    return (f"n = {int(df['n_total'].iloc[0])} voters, m = {M} candidates, "
            f"k = {_k_range()}, {int(df['trials'].iloc[0]):,} trials per cell")


def _smooth(values, window=SMOOTH_WINDOW):
    """Centred, edge-corrected moving average of `values` -- display only.

    The kernel is divided by the number of samples that really land on each point, so the
    two ends are honest averages of fewer points instead of being dragged towards zero, and
    no point is shifted in time. The raw values are what the CSV, the tables and the audit
    use; this only softens the Monte Carlo jitter in the drawings.
    """
    y = np.asarray(values, dtype=float)
    if window <= 1 or y.size == 0:
        return y
    w = window if window % 2 == 1 else window + 1        # keep the window centred
    if y.size < w:
        return y
    kernel = np.ones(w)
...
def _smooth_note(fig):
    """State on the figure itself that the drawn curves are smoothed."""
    if SMOOTH_WINDOW > 1:
        fig.text(0.006, 0.006, f"drawn curves: centred {SMOOTH_WINDOW}-point mean "
                               f"(display only -- data, tables and tests use the raw values)",
...
def _two_block_legend(fig, k_values, loc=(0.5, 0.005), ncol=None):
    ...
    fig.legend(handles=handles, loc="lower center",
               ncol=ncol or min(len(handles), 6), frameon=True, bbox_to_anchor=loc)
    _smooth_note(fig)
# ======================================================================================
# 8. FIGURES
# ======================================================================================
...
def fig_intensity_grid(df, target, metric, out_png, out_pdf, ylabel, suptitle):
    ...
    fig, axes = plt.subplots(2, 3, figsize=(18.4, 10.4), sharey=True)
    for ax, cult in zip(axes.flatten(), PANEL_ORDER):
        ...
        for k in COMMITTEE_SIZES:
            for rule in RULES:
                ...
                ys = _smooth(d[metric].to_numpy())
                ax.plot(d["manip_pct"].to_numpy(), ys, color=RULE_COLORS[rule],
                        linestyle=K_LINESTYLES[k], linewidth=1.7, zorder=3)
                pick = d["manip_pct"].isin(MARK_AT).to_numpy()
                ax.plot(d["manip_pct"].to_numpy()[pick], ys[pick], linestyle="none",
                        marker=K_MARKERS[k], markersize=6, color=RULE_COLORS[rule],
...
def fig_dual_metric(df, out_png, out_pdf, suptitle):
    ...
    for ax, cult in zip(axes.flatten(), PANEL_ORDER):
        ...
        for rule in RULES:
            ...
            c = RULE_COLORS[rule]
            ax.fill_between(mean.index, _smooth(lo["Change_Rate"]), _smooth(hi["Change_Rate"]),
                            color=c, alpha=0.15, lw=0)
            ax.fill_between(mean.index, _smooth(lo["Success_Rate"]), _smooth(hi["Success_Rate"]),
                            color=c, alpha=0.08, lw=0)
            ax.plot(mean.index, _smooth(mean["Change_Rate"]), color=c, lw=2.6,
                    marker=RULE_MARKERS[rule], markevery=[0, 12, 24, 36, 48],
                    markersize=6, label=RULE_LABELS[rule])
            ax.plot(mean.index, _smooth(mean["Success_Rate"]), color=c, lw=1.9, linestyle="--",
...
def fig_heatmap(df, out_png, out_pdf, suptitle):
    """Attack-success map: rows = culture x k, columns = manipulation bins, one panel per
    rule. Colour = bottom-target success rate (%)."""
    ...
    cmap = LinearSegmentedColormap.from_list("wr", ["#FFFFFF", "#FDD49E", "#FC8D59",
                                                    "#D7301F", "#7F0000"])
    n_rows_all = len(PANEL_ORDER) * len(COMMITTEE_SIZES)
    fig, axes = plt.subplots(1, 3, figsize=(19, max(7.5, 0.30 * n_rows_all + 2.4)),
                             sharey=True)
    ...
    for ax, rule in zip(axes, RULES):
        ...
        if rule == RULES[0]:
            n_k = len(COMMITTEE_SIZES)
            ax.set_yticklabels([lab.split("  ·  ")[0] if i % n_k == 0 else ""
                                for i, lab in enumerate(row_labels)], fontsize=7.5)
            for i, lab in enumerate(row_labels):
                ax.text(-0.02, i, lab.split("·")[-1].strip(),
    ...
    fig.suptitle(f"{suptitle}\nrows = culture ({len(COMMITTEE_SIZES)} committee sizes each), "
...
def fig_tipping_points(df, out_png, out_pdf, suptitle, threshold=50.0):
    ...
    n_rows = len(PANEL_ORDER) * len(COMMITTEE_SIZES)
    fig, ax = plt.subplots(figsize=(13.0, 0.42 * n_rows + 2.6))
    yticks, ylabels = [], []
    y = 0
    ...
    ax.text(MANIP_PCT_MAX + 4, n_rows - 0.5,
...
def fig_crossing_vs_k(df, out_png, out_pdf, suptitle, threshold=50.0):
    ...
    for ax, cult in zip(axes.flatten(), PANEL_ORDER):
        ...
        ax.set_xlim(min(COMMITTEE_SIZES) - 0.3, max(COMMITTEE_SIZES) + 0.3)
        ax.set_ylim(0, 52)
    for ax in axes[:, 0]:
...
def fig_aggregate(df, out_png, out_pdf, suptitle):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14.5, 10.4), sharey=True)
        ...
        for c, metric in enumerate(["Change_Rate", "Success_Rate"]):
            ...
            for k in COMMITTEE_SIZES:
                for rule in RULES:
                    ...
                    ax.plot(d.index, _smooth(d.values), color=RULE_COLORS[rule], lw=2.2,
    ...
    for ax, cult in zip(axes.flatten(), PANEL_ORDER):
        ...
        for k in COMMITTEE_SIZES:
            for rule in RULES:
                ...
                ax.plot(d["manip_pct"], _smooth(d["gap"]), color=RULE_COLORS[rule], lw=2.0,
                        linestyle=K_LINESTYLES[k])
...
def fig_advantage(df, out_png, out_pdf, suptitle):
    ...
    for ax, cult in zip(axes.flatten(), PANEL_ORDER):
        ...
        for k in COMMITTEE_SIZES:
            ...
            ax.plot(piv.index, _smooth(piv["Evaluative Voting"] - piv["k-Median Rule"]),
                    color=K_COLORS[k], lw=2.2, label=f"vs k-Median, {K_LABELS[k]}")
            ax.plot(piv.index, _smooth(piv["Evaluative Voting"] - piv["k-Median-Shapley"]),
