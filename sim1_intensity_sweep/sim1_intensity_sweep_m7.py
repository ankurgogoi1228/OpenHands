...
DESIGN
    ...
    candidates              m   = 7
    committee sizes         k   = 2, 3, 4, 5, 6   (k = 1 is a single winner, k = m takes all)
...
SCORE DISCIPLINE (STRICT)
    ...
    - Rank -> score map: (5, 4, 3, 2, 0) for m = 5 (the paper's map) and (5, 4, 3, 2, 1, 0, 0)
      for m = 7 -- six grades cannot rank seven candidates without a tie, so the two worst
      ranks share the bottom grade. See RANK_MAPS in the configuration block.
...
Command line
------------
    python sim1_intensity_sweep_m7.py                     # full run, 1000 trials per cell
    python sim1_intensity_sweep_m7.py --trials 100        # quick test run
    python sim1_intensity_sweep_m7.py --skip-figures      # data and tables only
    python sim1_intensity_sweep_m7.py --skip-simulation   # re-plot an existing CSV
========================================================================================
"""
import argparse
import itertools
...
COMMITTEE_SIZES = [2, 3, 4, 5, 6]  # k = 1 is a single winner, k = m takes everything
...
# the m = 7 map repeats the bottom grade, because six grades cannot rank seven candidates
# without at least one tie. Both maps stay strictly non-increasing and use every grade.
...
def _k_range():
    """'2,3,4,...' label for the committee sizes actually simulated."""
